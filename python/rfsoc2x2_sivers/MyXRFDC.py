...
with open(os.path.join(_THIS_DIR, 'xrfdc_functions.h'), 'r') as f:
    header_text = f.read()
...
class RFdc(pynq.DefaultIP):
    ...
    def __init__(self, description):
        ...
        if 'parameters' in description:
            from MyConfig import populate_config
    ...
    def runMTS(self):
        print('MTS initilization starts...')


                
        print('DAC SYNC LOOP')
        self._DAC_Sync_Config = _ffi.new("XRFdc_MultiConverter_Sync_Config*")   
        _lib.XRFdc_MultiConverter_Init(self._DAC_Sync_Config,_ffi.new('int*'),_ffi.new('int*'), 0)
        ...
        while True:
            ...
            statusMTSDAC = _lib.XRFdc_MultiConverter_Sync(self._instance,_lib.XRFDC_DAC_TILE,self._DAC_Sync_Config)        
            print('Status:' + str(statusMTSDAC) + ', DAC 0: Latency:' + str(self._DAC_Sync_Config.Latency[0])+ ' Offset:'
                  + str(self._DAC_Sync_Config.Offset[0]) +', DAC 2: Latency:' + str(self._DAC_Sync_Config.Latency[1])+ ' Offset:'
                  + str(self._DAC_Sync_Config.Offset[1])
                 )   
            ...
            if statusMTSDAC == 0 and self._DAC_Sync_Config.Offset[0] == self._DAC_Sync_Config.Offset[1]:
                print("DAC Multi-Tile-Sync completed successfully.")
                break
           
            if cnt==cntMax:
                print("Maximum number of attempts is reached.")
                break
                
        print('ADC SYNC LOOP')
        self._ADC_Sync_Config = _ffi.new('XRFdc_MultiConverter_Sync_Config*')
        _lib.XRFdc_MultiConverter_Init(self._ADC_Sync_Config,_ffi.new('int*'),_ffi.new('int*'), 0)
        self._ADC_Sync_Config.Tiles = 0x5   
        cnt = 0
        while True:
            cnt = cnt + 1
          
            statusMTSADC = _lib.XRFdc_MultiConverter_Sync(self._instance,_lib.XRFDC_ADC_TILE,self._ADC_Sync_Config)
            print('Status:' + str(statusMTSADC) + ', ADC 0: Latency:' + str(self._ADC_Sync_Config.Latency[0])+ ' Offset:'
                  + str(self._ADC_Sync_Config.Offset[0]) +', ADC 2: Latency:' + str(self._ADC_Sync_Config.Latency[2])+ ' Offset:'
                  + str(self._ADC_Sync_Config.Offset[2])
                 )  
            ...
            if statusMTSADC == 0 and self._ADC_Sync_Config.Offset[0] == self._ADC_Sync_Config.Offset[2]:
                print("ADC Multi-Tile-Sync completed successfully.")
                break
        ...
        for i in range(0,4):
            if (1<<i) & self._DAC_Sync_Config.Tiles:
                print("DAC{}: Latency(T1) = {}, Adjusted Delay Offset() = {}, Marker Delay = {}".format(i, self._DAC_Sync_Config.Latency[i],
                         self._DAC_Sync_Config.Offset[i], self._DAC_Sync_Config.Marker_Delay))
        ...
        for i in range(0,4):
            if (1<<i) & self._ADC_Sync_Config.Tiles:
                print("DAC{}: Latency(T1) = {}, Adjusted Delay Offset() = {}, Marker Delay = {}".format(i, self._ADC_Sync_Config.Latency[i],
                         self._ADC_Sync_Config.Offset[i], self._ADC_Sync_Config.Marker_Delay))
                print("=== MTS DAC Tile{} PLL Report ===".format(i))
    ...
    def printLatency(self):
        for i in range(4):
            print("ADC " + str(i) + ' Latency:' + str(self._ADC_Sync_Config.Latency[i]))
            print("ADC " + str(i) + ' Offset:'  + str(self._ADC_Sync_Config.Offset[i]))
            
        for i in range(4):
            print("DAC " + str(i) + ' Latency:' + str(self._DAC_Sync_Config.Latency[i]))
            print("DAC " + str(i) + ' Offset:'  + str(self._DAC_Sync_Config.Offset[i]))
    
    def sysrefDisable(self):
        status3 = _lib.XRFdc_MTS_Sysref_Config(self._instance, self._DAC_Sync_Config, self._ADC_Sync_Config, 0)   
        if status3 == _lib.XRFDC_MTS_OK:
            print('SYSREF is disabled.')

    def sysrefEnable(self):
        status3 = _lib.XRFdc_MTS_Sysref_Config(self._instance, self._DAC_Sync_Config, self._ADC_Sync_Config, 1) 
        if status3 == _lib.XRFDC_MTS_OK:
            print('SYSREF is enabled.')
    def _call_function(self, name, *args):
